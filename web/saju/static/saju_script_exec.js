...
function analyzeSaju() {
  ...
  SajuAPI.calcSaju(birthDatetime, calendarType, timeType)
    .then((apiResult) => {
      ...
      // Python API로 대운 계산 (양력 날짜 전달, calendar_type은 solar로)
      SajuAPI.calcDaeun(
        daeuBirthDatetime,
        gender,
        monthGan,
        ...
        // Python API로 연운 계산 (출생년도부터 110세까지)
        ...
        return SajuAPI.calcYeonun(year, year + 55, yearRange).then((yeonun) => {
          console.log('Python API 연운 결과:', yeonun);

          // Python API로 월운 계산
          return SajuAPI.calcWolun(new Date().getFullYear()).then((wolun) => {
            console.log('Python API 월운 결과:', wolun);
            console.log('월운 데이터 타입:', typeof wolun);
            console.log('월운 첫 번째 항목:', wolun[0]);
            ...
            // 강화된 형충파해 분석
            const hyungchungEnhanced =
              window.SajuSinsalExtended.analyzeHyungChungEnhanced(saju);

            // 십이신살 분석
            const sibiSinsal =
              window.SajuSinsalExtended.analyzeSibiSinsal(saju);

            // 길신류 분석
            const gilsin = window.SajuSinsalExtended.analyzeGilsin(
              saju,
              dayGan
            );

            // 그외 신살 분석
            const extraSinsal = window.SajuSinsalExtended.analyzeExtraSinsal(
              saju,
              dayGan
            );

            // displayBasicSaju에 모든 신살 정보 전달
            displayBasicSaju(
              saju,
              sibiun,
              yearGan,

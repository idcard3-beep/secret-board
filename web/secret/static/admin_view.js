...
// 페이지를 떠날 때 티켓 상태 검증
function validateTicketStatusOnUnload() {
  if (!ticketId) {
    console.log('⚠️ 티켓 ID가 없어 상태 검증을 건너뜁니다.');
    return;
  }
  
  console.log('🔍 페이지를 떠나기 전 티켓 상태 검증 시작...');
  
  // navigator.sendBeacon을 사용하여 페이지를 떠날 때도 요청 전송
  const url = `/secret/api/v1/admin/tickets/${ticketId}/validate-status`;
  const data = JSON.stringify({});
  
  if (navigator.sendBeacon) {
    // sendBeacon 사용 (비동기, 페이지 종료 후에도 전송 보장)
    const blob = new Blob([data], { type: 'application/json' });
    navigator.sendBeacon(url, blob);
    console.log('✅ sendBeacon으로 상태 검증 요청 전송');
  } else {
    // sendBeacon이 지원되지 않는 경우 fetch 사용 (동기)
    fetch(url, {
      method: 'POST',
      ...
      console.warn('⚠️ 상태 검증 요청 실패 (무시됨):', err);
    });
    console.log('✅ fetch로 상태 검증 요청 전송');
  }
}

// 페이지를 떠날 때 이벤트 리스너 등록
window.addEventListener('beforeunload', validateTicketStatusOnUnload);

// 페이지 숨김 이벤트 (모바일 등에서도 동작)
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    validateTicketStatusOnUnload();
  }
});

function load() {
  const url = `/secret/api/v1/admin/tickets/${ticketId}`;
...
function sendAdminReply() {
  ...
  const url = `/secret/api/v1/admin/tickets/${ticketId}/reply`;
...
function saveMessage(msgId, newContent, originalContent) {
  ...
  const url = `/secret/api/v1/admin/messages/${msgId}`;

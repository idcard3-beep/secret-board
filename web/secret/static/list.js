    ...
    .then((res) => {
      ...
      return res.json().then((data) => {
        if (!res.ok) {
          ...
          console.error('❌ API 에러 응답:', errorMsg);
...
function confirmPassword() {
  ...
  fetch(`/secret/api/v1/tickets/${ticketIdToVerify}/verify`, {
    method: 'POST',
    ...
    .then((result) => {
      if (result.ok) {
        ...
        // view.html에서 비밀번호 입력 폼이 나오지 않도록 플래그 설정
        sessionStorage.setItem('password_verified_' + ticketIdToVerify, 'true');
        
        closePasswordModal();
        console.log('🚀 페이지 이동:', `/view?id=${ticketIdToVerify}`);
        location.href = `/secret/view?id=${ticketIdToVerify}`; // 백업된 ID 사용
      } else {
        // 비밀번호가 틀리면 에러 메시지 표시

      ...
      (() => {
        ...
        form.addEventListener('submit', async (e) => {
          ...
          try {
            let response;
            // Mixed Content 오류 방지: 안전한 프로토콜 사용
            let apiBase = getApiUrl('/secret/api/v1/smembers');
            
            // 프로덕션 환경에서 HTTP가 감지되면 강제로 HTTPS로 변경
            const host = window.location.host;
            if (host !== 'localhost' && host !== '127.0.0.1' && !host.includes('localhost:')) {
              if (apiBase.startsWith('http://')) {
                console.warn('⚠️ HTTP URL 감지, HTTPS로 강제 변경:', apiBase);
                apiBase = apiBase.replace(/^http:\/\//, 'https://');
              }
              // 최종 검증: HTTPS가 아니면 강제 변경
              if (!apiBase.startsWith('https://')) {
                console.error('❌ HTTPS가 아닌 URL 감지, 강제 변경:', apiBase);
                apiBase = 'https://' + host + '/secret/api/v1/smembers';
              }
            }
            
            console.log('🔗 API 요청 URL (최종):', apiBase);
            console.log('🔒 사용 프로토콜:', getSecureProtocol());
            console.log('🌐 호스트:', host);
            console.log('🔍 URL 검증:', apiBase.startsWith('https://') ? '✅ HTTPS' : '❌ HTTP');
            
            if (memberId) {
              // 수정 모드
              const putUrl = `${apiBase}/${memberId}`;
              ...
              // 생성 모드
              console.log('📤 POST 요청 URL:', apiBase);

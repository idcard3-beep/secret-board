...
function initCalendarScripts(){var todayBtn=document.getElementById('today-btn');if(todayBtn){var newBtn=todayBtn.cloneNode(true);todayBtn.parentNode.replaceChild(newBtn,todayBtn);newBtn.addEventListener('click',function(){var now=new Date();var form=document.getElementById('today-form');if(form){document.getElementById('today-year').value=now.getFullYear();document.getElementById('today-month').value=now.getMonth()+1;var navInput=form.querySelector('input[name="nav"]');if(!navInput){navInput=document.createElement('input');navInput.type='hidden';navInput.name='nav';form.appendChild(navInput);}

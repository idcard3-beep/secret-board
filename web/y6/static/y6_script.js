...
resetHexagram(){this.hexagram=[];this.movingLines=[];this.currentPosition=0;this.najiaResult=null;this.consultationInfo={};document.querySelectorAll('.line-slot').forEach((slot)=>{slot.className='line-slot';});document.getElementById('questionType').value='';document.getElementById('questionText').value='';document.getElementById('consultantName').value='';document.getElementById('birthDate').value='';document.querySelectorAll('[class^="extra-slot"], [class*="extra-slot"]').forEach((slot)=>{slot.textContent='';slot.innerHTML='';});document.getElementById('analyzeBtn').disabled=true;document.getElementById('currentLineText').textContent='초효 (1효)';document.getElementById('analysisSection').style.display='none';const resultWrap=document.getElementById('resultWrap');if(resultWrap){resultWrap.classList.add('hidden');}
const existingSummary=document.querySelector('.hexagram-summary');if(existingSummary){existingSummary.remove();}

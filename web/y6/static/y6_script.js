...
displayCurrentSaju(data){const{saju,formatted_time}=data;document.getElementById('currentTimeDisplay').innerHTML=`
        ...
        `;document.getElementById('yearPillar').textContent=saju.year;document.getElementById('monthPillar').textContent=saju.month;document.getElementById('dayPillar').textContent=saju.day;document.getElementById('hourPillar').textContent=saju.hour;document.getElementById('sajuTimeInfo').innerHTML=`
        ...
        `;document.getElementById('sajuPillars').style.display='block';document.querySelector('.pillar-container').style.opacity='0';setTimeout(()=>{document.querySelector('.pillar-container').style.transition='opacity 0.5s ease';document.querySelector('.pillar-container').style.opacity='1';},100);}
showSajuError(error){document.getElementById('currentTimeDisplay').innerHTML=`
...
resetHexagram(){this.hexagram=[];this.movingLines=[];this.currentPosition=0;this.najiaResult=null;document.querySelectorAll('.line-slot').forEach((slot)=>{slot.className='line-slot';});document.getElementById('analyzeBtn').disabled=true;document.getElementById('currentLineText').textContent='초효 (1효)';document.getElementById('analysisSection').style.display='none';}
async analyzeHexagram(){try{this.getConsultationInfo();if(!this.currentSaju){alert('사주 정보를 불러오는 중입니다. 잠시 후 다시 시도해주세요.');return;}
const monthBranch=this.currentSaju.month_branch;const dayStem=this.currentSaju.day_stem;const dayBranch=this.currentSaju.day_branch;const response=await fetch('/y6/calculate-najia',{method:'POST',headers:{'Content-Type':'application/json',},body:JSON.stringify({yao_input:this.hexagram,yue_jian:monthBranch,ri_gan:dayStem,ri_chen:dayBranch,}),});const data=await response.json();if(data.success){this.najiaResult=data.result;this.displayNajiaResult();document.getElementById('analysisSection').style.display='block';document.getElementById('analysisSection').scrollIntoView({behavior:'smooth'});}else{alert(`분석 중 오류가 발생했습니다: ${data.error}`);}}catch(error){console.error('Analysis error:',error);alert(`분석 중 오류가 발생했습니다: ${error.message}`);}}
...
displayNajiaResult(){document.getElementById('hexagramName').textContent=this.najiaResult.main_info;document.getElementById('hexagramMeaning').textContent=`점시: 월건 ${this.najiaResult.base_info['월건']}, 일진 ${this.najiaResult.base_info['일진']}, 세효: ${this.najiaResult.base_info['세효']}효`;const resultContainer=document.getElementById('najiaResultContainer');resultContainer.innerHTML=`
